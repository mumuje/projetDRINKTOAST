...
function openSocketConnection() {
  ...
  socket.addEventListener("message", (event) => {
    ...
    /*******************************************/
    // message du serveur gameStaUpdate
    if (data.type === "gameStateUpdate") {
      ...
      currentPlayer2 = null;
      /*******************************************/
      // message du serveur coutdown
    } else if (data.type === "COUNTDOWN") {
      // Démarrer le compte à rebours
      startCountdown(data.start, data.duration);
      /*******************************************/
      // message du serveur playerSelectedResponse
      ...
      /*******************************************/
      // message du serveur playerSelectedResponseRED
      ...
      /*******************************************/
      // message du serveur STOP_COUTDOWN
    } else if (data.type === "STOP_COUNTDOWN") {
      stopCountdown();
      /*******************************************/
      // message du serveur redirect
      ...
      /*******************************************/
      // message du serveur NOSTARTPARTY
    } else if (data.type === "NOSTARTPARTY") {
      showMessage(data.content, 3000);
      dropzone.style.visibility = "hidden";
      /*******************************************/
      // message du serveur CHOOSEPLAYERSVIOLET
    } else if (data.type === "CHOOSEPLAYERSVIOLET") {
      dropzone.style.visibility = "hidden";
      choosePlayersViolet(data.players);
      /*******************************************/
      // message du serveur miniGameSelected
      ...
      if (currentGame === "TicTacToe") {
        ...
        if (data.TicTacToeAlready === false) {
          ...
          if (existingTable) {
            ...
            var savedTableState = JSON.parse(
              localStorage.getItem("tableState")
            );
            if (savedTableState) {
              table = document.createElement("table");
              table.style.borderCollapse = "collapse";
      ...
      if (currentGame === "TicTacToe") {
        ...
        player2 = data.player2;
        if (
          pseudo !== currentPlayerPURPLE &&
          (pseudo === player1 || pseudo === player2)
        ) {
          showMessage(data.content, 3000);
          dropzone.style.visibility = "hidden";
          // showMessage(data.content, 3000);
...
let voteCorrectButtonElement = document.getElementById(
